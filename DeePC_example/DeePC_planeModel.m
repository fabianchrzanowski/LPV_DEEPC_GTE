...
Outputs = {u, y, g};
options = sdpsettings('solver', 'mosek', 'verbose', 1, 'debug', 0); %, 'osqp.eps_abs', 1e-8, 'osqp.eps_rel', 1e-8);
...
%% --- ANALYSIS: Hankel Rank & Input Frequency (FFT Shifted) ---

% 1. Check Rank of the Hankel Matrices
% This confirms if your offline data (Up/Uf) is persistently exciting.
% Ideally, these should have full row rank.
r_Up = rank(Up);
r_Uf = rank(Uf);
r_Total = rank([Up; Uf]);

fprintf('\n--- Hankel Matrix Rank Analysis ---\n');
fprintf('Rank(Up): %d (Rows: %d)\n', r_Up, size(Up, 1));
fprintf('Rank(Uf): %d (Rows: %d)\n', r_Uf, size(Uf, 1));
fprintf('Rank([Up; Uf]): %d (Rows: %d)\n', r_Total, size([Up; Uf], 1));

% 2. Plot Frequency Spectrum (FFT Shifted)
% Analyzing the control input 'u' generated during the simulation
Fs = 1/Ts;              % Sampling Frequency
L_sig = size(u, 2);     % Length of signal (simLen)

figure('Name', 'Input Frequency Spectrum (Centered)');

for i = 1:nu
    % Extract current input channel
    signal = u(i, :);
    
    % --- Your Requested Method ---
    % 1. Compute FFT and shift it
    U_spectrum = abs(fft(signal));
    U_spectrum = fftshift(U_spectrum);
    
    % 2. Generate centered frequency vector
    % This maps the frequencies to the range [-Fs/2, Fs/2]
    frequencies = (-L_sig/2 : L_sig/2 - 1) * (Fs / L_sig);
    
    % Plot
    subplot(nu, 1, i);
    plot(frequencies, U_spectrum, 'LineWidth', 1.2);
    title(['Spectrum of Input u_{', num2str(i), '}']);
    xlabel('Frequency (Hz)');
    ylabel('Magnitude');
    grid on;
    
    % Optional: Zoom in near DC if needed (adjust as you see fit)
    % xlim([-Fs/2, Fs/2]); 
end