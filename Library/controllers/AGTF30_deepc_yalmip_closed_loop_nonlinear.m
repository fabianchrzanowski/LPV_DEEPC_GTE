...
solver_name = 'ipopt'; % Change to 'quadprog' if you have a QP solver available
...
for t = T_ini+1:N_total
    ...
    if strcmpi(solver_name,solver_name)
    ...
    if diagnostics.problem ~= 0
        disp(diagnostics.info)          % YALMIP's description of the problem
        disp(yalmiperror(diagnostics.problem))  % human-readable error
    end
