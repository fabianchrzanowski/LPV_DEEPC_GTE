...
% ========================================================================
% LPV-DeePC: Disturbance Rejection Test
%
% Applies disturbances that can be configured. Tests the disturbance
% rejection and tracking recovery
% Uses CasADi + IPOPT.
% ========================================================================
...
if exist('OVERRIDE_WF_DECIMATION', 'var')
    wf_decimation_factor = OVERRIDE_WF_DECIMATION;
else
    wf_decimation_factor = 1;   % Use all 32 Hankels by default
end
n_sys = 4;

%% ===== Disturbance Configuration =====
% Output disturbance: additive bias on the measured N_H (simulates sensor
% bias, unmodeled load change, or ambient condition shift).
%   Each row: [start_step, end_step, magnitude_rpm]
%   Steps are relative to control start (si = 1, 2, ..., N_sim)
DIST_OUTPUT = [
    round(N_sim*0.20), round(N_sim*0.35), +500;    % positive output disturbance at 20-35%
    round(N_sim*0.55), round(N_sim*0.70), -400;    % negative output disturbance at 55-70%
];

% Input disturbance: additive perturbation on the fuel flow AFTER the
% controller (simulates actuator fault or fuel delivery error).
%   Each row: [start_step, end_step, magnitude_pps]
DIST_INPUT = [
    % round(N_sim*0.40), round(N_sim*0.48), +0.15;   % positive input disturbance at 40-48%
    0;
];
...
% --- APPLY DATA DECIMATION / INDEX OVERRIDE ---
n_total_files = min(numel(filelist_train), numel(wf_values));
if exist('OVERRIDE_WF_INDEX', 'var') && ~isempty(OVERRIDE_WF_INDEX)
    keep_idx = OVERRIDE_WF_INDEX(:)';
    keep_idx = keep_idx(keep_idx >= 1 & keep_idx <= n_total_files);
    fprintf('[DISTURBANCE TEST] Using specific Wf indices: %s\n', mat2str(keep_idx));
else
    keep_idx = 1:wf_decimation_factor:n_total_files;
end
filelist_train = filelist_train(keep_idx);
wf_values = wf_values(keep_idx);
n_op = length(keep_idx);

fprintf('Building per-Wf Hankels (%d operating points, max %d columns each)...\n', n_op, max_cols_per_wf);
...
for k = 1:n_op
    data = load(fullfile(filelist_train(k).folder, filelist_train(k).name));
...
S = struct();
S.solver = nlpsol('deepc_dist', 'ipopt', nlp, opts);
...
y_true = nan(N_total,1);          % undisturbed plant output
u_applied = zeros(N_total,1);     % actual input after input disturbance
dist_y_hist = zeros(N_total,1);   % output disturbance applied
dist_u_hist = zeros(N_total,1);   % input disturbance applied
u_hist(1:T_ini) = u0;
u_applied(1:T_ini) = u0;
u_prev = u0;
...
% Control loop
wf_filtered = u0;
fprintf('LPV-DeePC disturbance rejection (%d steps)...\n', N_sim);
for t = T_ini+1:N_total
    ...
    for di = 1:size(DIST_OUTPUT,1)
        if si >= DIST_OUTPUT(di,1) && si <= DIST_OUTPUT(di,2)
            d_y = d_y + DIST_OUTPUT(di,3);
        end
    end
    d_u = 0;
    for di = 1:size(DIST_INPUT,1)
        if si >= DIST_INPUT(di,1) && si <= DIST_INPUT(di,2)
    ...
    % ---- STEP 3: Convert signals to deviation space ----
    % Subtract the operating-point means so the Hankel data is consistent.
    % Note: dy_ini uses the DISTURBED output — the controller doesn't know about d_y.
    u_mean_k = hankel_u_mean(k_sel);
    y_mean_k = hankel_y_mean(k_sel);

    du_ini = u_hist(t-T_ini:t-1) - u_mean_k;
    dy_ini = y_hist(t-T_ini:t-1) - y_mean_k;   % controller sees disturbed output
    dref_seg = ref(t:t+N_pred-1) - y_mean_k;
    du_prev = u_prev - u_mean_k;
    du0 = 0;  

    % ---- STEP 4: Load the Hankel matrices for the selected operating point ----
    ...
    % Pin zero-padded g elements to zero. Input bounds in deviation space.
    g_lb = -inf(S.n_cols, 1);
    g_ub =  inf(S.n_cols, 1);
    if nc_sel < S.n_cols
        g_lb(nc_sel+1:end) = 0;
    ...
    % ---- STEP 6: Solve the DeePC optimisation (warm-started) ----
    tic;
    ...
    % ---- STEP 8: Apply input disturbance AFTER the controller ----
    ...
    % ---- STEP 9: Simulate the nonlinear plant with the disturbed input ----
    t_step = (0:t)'*Ts;
    u_step = [u0; u_applied(1:t)];   % use actual applied inputs (with d_u)
    MWS.In.Tsim = t_step(end);
    MWS.In.Alt  = timeseries([0;0],[0 t_step(end)],'Name','Alt');
    MWS.In.MN   = timeseries([0;0],[0 t_step(end)],'Name','MN');
    MWS.In.dT   = timeseries([0;0],[0 t_step(end)],'Name','dT');
    ...
    simIn = simIn.setModelParameter('StartTime','0','StopTime',num2str(t_step(end)),'ReturnWorkspaceOutputs','on');
    simOut = sim(simIn);
    y_plant = measure_tracking_output(fetch_out_dyn(simOut), track_name);
    y_true(t) = y_plant;             % true undisturbed plant output
    y_hist(t) = y_plant + d_y;       % controller sees disturbed measurement
    ...
    if mod(si,20)==0 || si == 1
        ...
        fprintf('  Step %d/%d  Wf=%.3f -> Hankel #%d  solve=%.1fms%s\n', ...
            si, N_sim, u_prev, k_sel, solve_times(si)*1000, dist_str);
    end
end
...
mat_filename = sprintf('Results/results_lpv_deepc_disturbance_%s.mat', scen_tag);
controller_name = sprintf('LPV-DeePC (Disturbance) %s', SCENARIO);
save(mat_filename, ...
    'y_hist', 'y_true', 'u_hist', 'u_applied', ...
    'dist_y_hist', 'dist_u_hist', ...
    'wf_scheduler_hist', 'solve_times', ...
    'time', 'ref_plot', 'controller_name', 'track_name', ...
    'DIST_OUTPUT', 'DIST_INPUT');
fprintf('Saved results to %s\n', mat_filename);

%% ===== Plot Results =====
control_time = time(T_ini+1:end);
ct_idx = T_ini+1:N_total;

figure('Name','LPV-DeePC Disturbance Rejection')

% --- Subplot 1: Tracking ---
subplot(4,1,1)
plot(control_time, ref_plot(ct_idx), '--', 'LineWidth',1.2, 'Color',[0.5 0.5 0.5])
hold on
plot(control_time, y_hist(ct_idx), 'Color', [0.00 0.45 0.74], 'LineWidth', 1.8)
% Shade disturbance regions
for di = 1:size(DIST_OUTPUT,1)
    t1 = (DIST_OUTPUT(di,1)-1+T_ini)*Ts;
    t2 = (DIST_OUTPUT(di,2)-1+T_ini)*Ts;
    yl = ylim;
    patch([t1 t2 t2 t1], [yl(1) yl(1) yl(2) yl(2)], ...
        [1 0.8 0.8], 'FaceAlpha',0.25, 'EdgeColor','none');
end
for di = 1:size(DIST_INPUT,1)
    t1 = (DIST_INPUT(di,1)-1+T_ini)*Ts;
    t2 = (DIST_INPUT(di,2)-1+T_ini)*Ts;
    yl = ylim;
    patch([t1 t2 t2 t1], [yl(1) yl(1) yl(2) yl(2)], ...
...
legend('Reference', 'Measured output (disturbed)', 'Location', 'best')
title('Tracking with Disturbances')

% --- Subplot 2: Control input ---
subplot(4,1,2)
stairs(control_time, u_hist(ct_idx), 'b', 'LineWidth',1.4)
hold on
stairs(control_time, u_applied(ct_idx), 'r--', 'LineWidth',1.0)
grid on
xlabel('Time [s]')
ylabel('W_f [pps]')
...
subplot(4,1,3)
yyaxis left
stairs(control_time, dist_y_hist(ct_idx), 'r', 'LineWidth',1.4)
ylabel('Output dist. [RPM]')
yyaxis right
stairs(control_time, dist_u_hist(ct_idx), 'b', 'LineWidth',1.4)
...
subplot(4,1,4)
err_true = y_true(ct_idx) - ref_plot(ct_idx);
plot(control_time, err_true, 'b', 'LineWidth',1.2)
hold on
yline(0, 'k--')
grid on
xlabel('Time [s]')
...
sgtitle('LPV-DeePC Disturbance Rejection Test', 'FontSize', 14, 'FontWeight', 'bold')

%% ===== Summary =====
y_ctrl = y_true(T_ini+1:end);
