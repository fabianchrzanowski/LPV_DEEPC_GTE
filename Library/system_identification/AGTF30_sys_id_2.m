% ========================================================================
% LPV system id: system identification
% 
% at each operating point, we only consider for identification the data
% obtained at that operating point.
%
% What other models can be found that describe the dynamics 
% 
% ========================================================================
%
%
...
for f = 1:nf
    ...
    sol_op = sdpsettings('solver','mosek','verbose',2);
...
%     end
    ...
    if any(~isfinite(regressors(:))) || any(~isfinite(LHS(:)))
        error('Non-finite values found in regressors/LHS at file index %d',f);
    end

    sol = value(theta);
    if info.problem ~= 0 || any(~isfinite(sol(:)))
        warning('Identification optimizer failed at file %d (code %d). Falling back to least-squares.',f,info.problem);
        sol = LHS * pinv(regressors);
    end

    if any(~isfinite(sol(:)))
        error('Non-finite identified coefficients at file index %d after fallback.',f);
    end

    SXY = blkdiag(diag(data_x.scale.max(:,f)),diag(data_y.scale.max(:,f)));
    SU  = diag(data_u.scale.max(:,f));
    SZ  = diag(data_z.scale.max(:,f));
    if any(~isfinite(SXY(:))) || any(~isfinite(SU(:))) || any(~isfinite(SZ(:)))
        error('Non-finite scaling matrices at file index %d',f);
    end
    ...
    if any(~isfinite(A_dt(:,:,f)),"all") || any(~isfinite(B_dt(:,:,f)),"all") || ...
       any(~isfinite(C_dt(:,:,f)),"all") || any(~isfinite(D_dt(:,:,f)),"all")
        error('Non-finite state-space matrices generated at file index %d',f);
    end
